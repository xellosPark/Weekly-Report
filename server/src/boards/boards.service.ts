  ...
  getBoards(id: number) {
    ...
    .then((board) => {
      if (!board) {
       ...
       const updateAtKST = new Date(board.updateAt).toLocaleString('ko-KR', { timeZone: 'Asia/Seoul' });
...
async createBoard(createBoardDto: CreateBoardDto) {
  ...
  const data = this.boardRepository.create({ id, title, description, isPublic })
